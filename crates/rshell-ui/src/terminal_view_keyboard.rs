...
pub(crate) fn connect_keyboard(
    ...
    let commit_input = sender.input_sender().clone();
    im_context.connect_commit(move |_, text| {
        let _ = commit_input.send(TerminalViewMsg::CommittedText(text.to_owned()));
    });
    let key = gtk::EventControllerKey::new();
    key.set_im_context(Some(im_context));
    let key_input = sender.input_sender().clone();
    let bindings = bindings.to_vec();
    key.connect_key_pressed(move |_, key, _, state| {
        if should_handle_key(key, state, &bindings) {
            if key_input.send(TerminalViewMsg::Key { key, state }).is_ok() {
    ...
    key.connect_key_released(move |_, key, _, _| {
        let _ = release_input.send(TerminalViewMsg::KeyReleased(key));
    });
    ...
    let focus_input = sender.input_sender().clone();
    focus.connect_leave(move |_| {
        focus_out.focus_out();
        let _ = focus_input.send(TerminalViewMsg::FocusLost);
    });
    ...
    search_keys.connect_key_pressed(move |_, key, _, state| {
        if matches!(
            ...
            if search_input
                .send(TerminalViewMsg::Key { key, state })
                .is_ok()
            {
                glib::Propagation::Stop
            } else {
